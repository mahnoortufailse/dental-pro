...
import { connectDB, Billing } from "@/lib/db"
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { patientId, treatments, totalAmount, paidAmount, paymentStatus, notes } = await request.json()
    if (!patientId || !totalAmount) return NextResponse.json({ error: "Patient and total amount required" }, { status: 400 })
