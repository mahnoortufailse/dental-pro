...
export async function POST(request: NextRequest) {
	try {
		await connectDB();
		const { username, email, password, name, role, phone, specialty } =
			await request.json();
		...
		if (role === "admin") {
			return NextResponse.json(
				{
					error:
						"Admin accounts cannot be created through signup. Contact system administrator.",
				},
				{ status: 403 }
			);
		}
		...
		if (phone) {
			const verificationLink = `${
				process.env.NEXT_PUBLIC_APP_URL || "https://dentalcare.app"
			}/verify?token=${token}`;
			...
			if (!whatsappResult.success) {
				console.warn(
					"  WhatsApp notification failed for signup:",
					whatsappResult.error
				);
				// Don't fail the signup if WhatsApp fails - log and continue
			} else {
				console.log(
					"  WhatsApp account confirmation sent:",
