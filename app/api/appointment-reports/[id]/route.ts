...
export async function PUT(
	...
	try {
		...
		const report = await AppointmentReport.findByIdAndUpdate(
			params.id,
			updates,
			{ new: true }
		)
		...
		console.error("  PUT appointment report error:", error);
...
export async function DELETE(
	...
	try {
		...
		if (
			payload.role === "doctor" &&
			report.doctorId.toString() !== payload.userId
		) {
			return NextResponse.json(
		...
		console.error("  DELETE appointment report error:", error);
		return NextResponse.json(
