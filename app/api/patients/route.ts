...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!insuranceProvider?.trim()) missingCriticalCredentials.push("Insurance Provider");
    ...
    if (!insuranceNumber?.trim()) missingNonCriticalCredentials.push("Insurance Number");
    ...
    const newPatient = await Patient.create({
      ...
      credentialStatus: missingNonCriticalCredentials.length === 0 ? "complete" : "incomplete",
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (updateData.insuranceNumber !== undefined || updateData.address !== undefined) {
      const patient = await Patient.findById(id);
      if (patient) {
        const mergedData = { ...patient.toObject(), ...updateData };
