	...
	try {
		...
		if (payload.role === "doctor") {
			...
			if (
				...
				!patient.assignedDoctorId._id.equals(doctorId)
			) {
				console.log(
...
export async function PUT(
	...
	try {
		...
		console.log(
			"  Missing non-critical credentials:",
			missingNonCriticalCredentials
		); // Debug log
		...
		if (
			...
			updateData.assignedDoctorId !== patient?.assignedDoctorId?.toString()
		) {
			...
			if (newDoctor && newDoctor.email && updatedPatient.email) {
				const { sendDoctorAssignmentEmail } = await import(
				...
				if (!emailResult.success) {
					...
					console.log(
						"  Doctor assignment email sent successfully:",
						emailResult.messageId
					);
				}
			} else {
				console.warn(
					"  Doctor or patient email not found — Doctor assignment email skipped"
				);
			}
		}
...
export async function DELETE(
	...
	try {
		...
		console.log(
			"  Deleted medical history records:",
			deletedMedicalHistory.deletedCount
		);
