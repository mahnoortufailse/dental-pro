...
export default function PatientsPage() {
	...
	const validatePatientCredentials = (data: any) => {
		...
		if (!data.insuranceProvider?.trim())
			criticalCredentials.push("Insurance Provider");
		...
		try {
			...
			if (res.ok) {
				...
				if (editingPatient) {
					...
					toast.success(
						"Patient added successfully! Strong password sent to email."
					);
				}
								...
								<div className="bg-card rounded-lg shadow-lg border border-border p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
									...
									<div className="space-y-3 mb-6">
										...
										<div>
											...
											<p className="text-foreground">
												{selectedPatient.assignedDoctorId?.name || "Unassigned"}
											</p>
										</div>
										<div>
											<p className="text-xs font-semibold text-muted-foreground uppercase">
									...
									{editingMedicalInfo && user?.role === "doctor" && (
										<form
											onSubmit={handleUpdateMedicalInfo}
